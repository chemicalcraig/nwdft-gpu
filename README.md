...
The core of the RT-TDDFT propagation involves solving the Liouville-von Neumann equation. In the Magnus expansion formalism (specifically the second-order Magnus propagator), the time-evolution operator $U(t)$ is approximated as an exponential of the Fock matrix $F(t)$. The update of the density matrix $P(t)$ is computed using the Baker-Campbell-Hausdorff (BCH) expansion:

$$ P(t+\Delta t) = e^{-i F \Delta t} P(t) e^{i F \Delta t} = P(t) + \frac{1}{1!} [-iF\Delta t, P(t)] + \frac{1}{2!} [-iF\Delta t, [-iF\Delta t, P(t)]] + \dots $$

This iterative expansion requires repeated dense matrix multiplications ($ZGEMM$) and matrix additions ($ZAXPY$). For a system with $N$ basis functions, each step scales as $O(N^3)$. We identified this dense linear algebra loop as the primary candidate for GPU offloading.

### 2.2. Implementation Details
The implementation follows a hybrid Fortran/C/CUDA architecture:

1.  **CUDA/C Wrapper Layer:** A C-based wrapper (`rttddft_gpu.cu`) interfaces directly with the NVIDIA cuBLAS library. It manages GPU memory allocation (`cudaMalloc`), data transfer (`cudaMemcpy`), and linear algebra operations (`cublasZgemm`, `cublasZaxpy`).
2.  **Fortran Interface:** A Fortran module (`rttddft_gpu.fh`) exposes these C functions to the legacy Fortran codebase using standard ISO C bindings or direct external linking, ensuring compatibility with NWChem's memory management.
3.  **Propagator Refactoring:** The core propagator routine `prop_magnus_exp_bch.F` was refactored. A conditional execution path was introduced:
    - **Rank 0 Offloading:** The MPI rank 0 process takes ownership of the GPU. It allocates device memory and transfers the density ($P$) and Fock ($F$) matrices to the device.
